CREATE OR REPLACE FORCE VIEW WORD_SEDI ( CI,
...
, Periodo.get_ultimo ('WORD_PERIODI_GIURIDICI','DAL','AL',
                      pegi.al,'#data_assunzione#GESTIONE#certificabile#SETTORE#SEDE#CI#',
			    '#'||pegi.data_assunzione||'#'||pegi.gestione|| '#' ||pegi.certificabile ||'#' || pegi.settore||                              '#' ||pegi.sede||'#'||pegi.ci||'#')
								           pegi_al
, qugi.dal                                 qugi_dal
, qugi.al                                  qugi_al
