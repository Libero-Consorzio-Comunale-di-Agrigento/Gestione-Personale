CREATE OR REPLACE PACKAGE PECCUD06 IS
/******************************************************************************
 NOME:          PECCUD06
 DESCRIZIONE:   Estrazione dati per STAMPA MODELLO CUD 2006 - Redditi 2005
...
END;
/
CREATE OR REPLACE PACKAGE BODY PECCUD06 IS
...
-- dbms_output.put_line('entro nel cursore');
        ...
        cursore_fiscale_06.PULISCI_TAB_REPORT_FINE_ANNO;
        cursore_fiscale_06.INSERT_TAB_REPORT_FINE_ANNO (D_anno,D_da_ci,D_a_ci);
        commit;
        LOCK TABLE TAB_REPORT_FINE_ANNO IN EXCLUSIVE MODE NOWAIT;
        FOR CUR_RPFA in cursore_fiscale_06.CUR_RPFA2 (D_anno
							   ,D_filtro_1
							   ,D_filtro_2
							   ,D_filtro_3
							   ,D_filtro_4
...
-- dbms_output.put_line('Fatta insert3 per il CI: '||CUR_RPFA.ci);
  ...
  IF nvl(D_previdenziale,'Y') != 'X' THEN

    FOR CUR_EMENS in cursore_fiscale_06.CUR_EMENS ( D_anno, CUR_RPFA.ci,'CUD','')  LOOP
...
-- dbms_output.put_line('SR: '||CUR_EMENS.specie_rapporto);
                   ...
                   IF CUR_EMENS.specie_rapporto = 'DIP' THEN
                       ...
                       ( no_prenotazione, no_passo, pagina, riga, testo )
                        ...
                        , rpad('C1',5,' ')||rpad(nvl(CUR_EMENS.posizione_inps,' '),16,' ')||
                          ...
                          rpad('C4',5,' ')||lpad(nvl(CUR_EMENS.imponibile,'0'),16,' ')||
                          ...
                          rpad('C8',5,' ')||lpad(nvl(CUR_EMENS.ritenuta,'0'),16,' ')||
                          rpad('C9',5,' ')||lpad(nvl(D_bonus_l243,'0'),16,' ')||
                   ...
                   IF CUR_EMENS.specie_rapporto = 'COCO' THEN
                        ...
                        , D_pagina_emens
                        , D_riga
                        , rpad('C12',5,' ')||lpad(nvl(CUR_EMENS.imponibile,'0'),16,' ')||
                          rpad('C13',5,' ')||lpad(nvl(CUR_EMENS.contributo,'0'),16,' ')||
                          rpad('C14',5,' ')||lpad(nvl(CUR_EMENS.ritenuta,'0'),16,' ')||
...
-- dbms_output.put_line('Tratto i dati DMA per il CI: '||CUR_RPFA.ci);
      ...
      FOR CUR_DMA IN cursore_fiscale_06.CUR_DMA (D_anno, CUR_RPFA.ci, '', 'X', 'N')
      LOOP
...
-- dbms_output.put_line('PG_IN1: '||D_pagina_dma);
        ...
        , rpad('C18',5,' ')||rpad(nvl(CUR_DMA.gest_cf,' '),16,' ')||
          ...
          rpad('C25',5,' ')||rpad(nvl(CUR_DMA.anno_rif,'0'),16,' ')||
          rpad('C26',5,' ')||rpad(nvl(CUR_DMA.ipn_pens,'0'),16,' ')||
          rpad('C27',5,' ')||rpad(nvl(CUR_DMA.contr_pens,'0'),16,' ')||
          rpad('C28',5,' ')||rpad(nvl(CUR_DMA.ipn_tfs,'0'),16,' ')||
          rpad('C29',5,' ')||rpad(nvl(CUR_DMA.contr_tfs,'0'),16,' ')||
          rpad('C30',5,' ')||rpad(nvl(CUR_DMA.ipn_tfr,'0'),16,' ')||
          rpad('C31',5,' ')||rpad(nvl(CUR_DMA.contr_tfr,'0'),16,' ')||
          rpad('C32',5,' ')||rpad(nvl(CUR_DMA.ipn_cassa_credito,'0'),16,' ')||
          rpad('C33',5,' ')||rpad(nvl(CUR_DMA.contr_cassa_credito,'0'),16,' ')||
          rpad('C34',5,' ')||rpad(nvl(CUR_DMA.ipn_enpdedp,'0'),16,' ')||
          rpad('C35',5,' ')||rpad(nvl(CUR_DMA.contr_enpdedp,'0'),16,' ')||
          rpad('C36',5,' ')||rpad(nvl(CUR_DMA.contr_calamita,'0'),16,' ')||
          '{'
        );
    END LOOP; -- CUR_DMA
                --
...
-- dbms_output.put_line('Tratto i dati IPOST per il CI: '||CUR_RPFA.ci);
		...
		EXCEPTION WHEN NO_DATA_FOUND THEN
                ...
                BEGIN
                  select min(dedp.dal), max(dedp.al)
                    ...
                    into d_min_dal, d_max_al, d_max_d_cess, d_max_c_cess
                ...
                BEGIN
                  select min(dedp.dal), max(dedp.al)
                    into d_min_dal_arr, d_max_al_arr
                    from denuncia_inpdap  dedp
                   where dedp.anno = D_anno
                     and dedp.dal <= D_fin_a
      ...
      FOR CUR_INPDAP IN cursore_fiscale_06.CUR_INPDAP (D_anno, CUR_RPFA.ci, D_ini_a, D_fin_a, '')
      LOOP
...
-- dbms_output.put_line('inserimento record: ');
        ...
        , D_pagina_inpdap
        , D_riga
        , rpad('C37',5,' ')||rpad(rpad(nvl(D_inpdap_comparto,' '),2,' ')||lpad(' ',1,' ')|| 
                             rpad(nvl(D_inpdap_sottocomparto,' '),2,' ')||lpad(' ',1,' ')||
                             rpad(nvl(D_inpdap_qualifica,' '),6,' '),16,' ')||
          rpad('C38',5,' ')||rpad(nvl(D_inpdap_cf,' '),16,' ')||
          rpad('C39',5,' ')||rpad(nvl(to_char(d_inpdap_data_decorrenza,'ddmmyyyy'),' '),16,' ')||
          rpad('C40',5,' ')||rpad(nvl(to_char(d_inpdap_dal,'ddmmyyyy'),' '),16,' ')||
          rpad('C41',5,' ')||rpad(nvl(to_char(D_inpdap_al,'ddmmyyyy'),' '),16,' ')||
          rpad('C42',5,' ')||lpad(nvl(d_inpdap_gg_tfr,'0'),16,' ')||
          rpad('C43',5,' ')||lpad(nvl(D_causa_cessazione,' '),16,' ')||
          rpad('C44',5,' ')||lpad(nvl(d_inpdap_imp,' '),16,' ')||
          rpad('C45',5,' ')||lpad(nvl(d_inpdap_serv,' '),16,' ')||
          rpad('C46',5,' ')||rpad(nvl(d_inpdap_cassa,' '),16,' ')||
          rpad('C47',5,' ')||lpad(nvl(d_inpdap_gg,'0'),16,' ')||
          '{'
        )
        ;
        D_riga:= D_riga + 1;
...
-- dbms_output.put_line('PG_IN2: '||D_pagina_inpdap);
        ...
        values
         ...
         , D_pagina_inpdap
         , D_riga
         ,rpad('C48',5,' ')||rpad(nvl(substr(nvl(d_inpdap_mag,' '),2,2),' '),16,' ')||
          rpad('C49',5,' ')||lpad(decode(substr(nvl(d_inpdap_mag,' '),2,2),null,' ',to_char(nvl(d_gg_mag_1,0))),16,' ')||
          rpad('C50',5,' ')||rpad(nvl(substr(nvl(d_inpdap_mag,' '),5,2),' '),16,' ')||
          rpad('C51',5,' ')||lpad(decode(substr(nvl(d_inpdap_mag,' '),5,2),null,' ',to_char(nvl(d_gg_mag_2,0))),16,' ')||
          rpad('C52',5,' ')||rpad(nvl(substr(nvl(d_inpdap_mag,' '),8,2),' '),16,' ')||
          rpad('C53',5,' ')||lpad(decode(substr(nvl(d_inpdap_mag,' '),8,2),null,' ',to_char(nvl(d_gg_mag_3,0))),16,' ')||
          rpad('C54',5,' ')||rpad(nvl(substr(nvl(d_inpdap_mag,' '),11,2),' '),16,' ')||
          rpad('C55',5,' ')||lpad(decode(substr(nvl(d_inpdap_mag,' '),11,2),null,' ',to_char(nvl(d_gg_mag_4,0))),16,' ')||
          rpad('C56',5,' ')||lpad(nvl(d_inpdap_fisse,'0'),16,' ')||
          rpad('C57',5,' ')||lpad(nvl(d_inpdap_acce,'0'),16,' ')||
          rpad('C58',5,' ')||lpad(nvl(d_inpdap_comp_18,'0'),16,' ')||
          rpad('C59',5,' ')||lpad(nvl(d_inpdap_inadel,'0'),16,' ')||
          rpad('C60',5,' ')||lpad(nvl(d_inpdap_tfr,'0'),16,' ')||
          rpad('C61',5,' ')||lpad(nvl(d_inpdap_premio,'0'),16,' ')||
          rpad('C62',5,' ')||lpad(nvl(d_inpdap_ind_non_a,'0'),16,' ')||
          '{'
          )
        ;
        D_riga:= D_riga + 1;
...
-- dbms_output.put_line('PG_IN3: '||D_pagina_inpdap);
        ...
        values
         ...
         , D_pagina_inpdap
         , D_riga
         ,rpad('C63',5,' ')||lpad(nvl(d_inpdap_l_165,'0'),16,' ')||
          rpad('C64',5,' ')||lpad(nvl(d_tredicesima,'0'),16,' ')||
          rpad('C65',5,' ')||rpad(nvl(to_char(d_data_opz_tfr,'ddmmyyyy'),' '),16,' ')||
          rpad('C66',5,' ')||rpad(nvl(D_cf_amm_fisse,' '),16,' ')||
          rpad('C67',5,' ')||rpad(nvl(D_cf_amm_acc,' '),16,' ')||
          rpad('C71',5,' ')||rpad(nvl(d_inpdap_l_388,' '),16,' ')||
          rpad('C72',5,' ')||lpad(nvl(d_iis_conglobata,'0'),16,' ')||
          rpad('C73',5,' ')||lpad(nvl(d_ipn_tfr,'0'),16,' ')||
          rpad('C74',5,' ')||lpad(decode( D_stampa_cont_sosp ,1, nvl(D_contr_sospesi_2002,'0'),'0'),16,' ')||
          rpad('C75',5,' ')||lpad(decode( D_stampa_cont_sosp ,1, nvl(D_contr_sospesi_2003,'0'),'0'),16,' ')||
          rpad('C76',5,' ')||lpad(decode( D_stampa_cont_sosp ,1, nvl(D_contr_sospesi_2004,'0'),'0'),16,' ')||
          rpad('C77',5,' ')||lpad(decode( D_stampa_cont_sosp ,1, nvl(D_contr_sospesi_2005,'0'),'0'),16,' ')||
          rpad('C78',5,' ')||lpad(nvl(D_inpdap_perc_l300,'0'),16,' ')||
          '{'
          )
        ;
...
-- dbms_output.put_line('PG_IN4: '||D_pagina_inpdap);
  ...
  values
       ...
       , D_pagina
       , D_riga
       , rpad('C68',5,' ')||lpad(nvl(D_contributi_inpdap,'0'),16,' ')||
         rpad('C69',5,' ')||lpad(nvl(D_contributi_tfs,'0'),16,' ')||
         rpad('C70',5,' ')||lpad(nvl(D_contributi_tfr,'0'),16,' ')||
      '{'
       )
       ;
