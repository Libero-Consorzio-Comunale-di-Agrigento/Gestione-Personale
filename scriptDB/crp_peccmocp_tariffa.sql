CREATE OR REPLACE PACKAGE Peccmocp_Tariffa IS
/******************************************************************************
 NOME:        PECCMOCP_TARIFFA
 DESCRIZIONE: Determinazione della Tariffa delle Voci a Quantità e a Calcolo - Previsione

 ANNOTAZIONI: Il Package contiene le procedure richiamete dda:
              -  Calcolo della retribuzione (PECCMOCP7).
...
revisione varchar2(30) := '2 del 30/09/2004';
...
CREATE OR REPLACE PACKAGE BODY Peccmocp_Tariffa IS
...
/******************************************************************************
 ...
 ANNOTAZIONI: Richiamata da Paccmocp7 e PECAVARI
 REVISIONI:
 Rev. Data       Autore Descrizione
 ---- ---------- ------ ------------------------------------------------------
 0    05/07/2004 MF     Prima emissione.
...
select P_tariffa 
  ...
  into P_tariffa
  from informazioni_retributive_bp r
...
/******************************************************************************
 ...
 ANNOTAZIONI: Richiamata da Peccmocp7
 REVISIONI:
 Rev. Data       Autore Descrizione
 ---- ---------- ------ ------------------------------------------------------
 0    05/07/2004 MF     Prima emissione.
...
/******************************************************************************
 ...
 ANNOTAZIONI: Richiamata da Paccmocp7
 REVISIONI:
 Rev. Data       Autore Descrizione
 ---- ---------- ------ ------------------------------------------------------
 0    05/07/2004 MF     Prima emissione.
...
select
  ...
  * sum( round( sum(gg_100) / P_gg_lavoro
       ...
       + round( sum(gg_per) / P_gg_lavoro
                            ...
                            * sum(gg_per * abs(quota) / intero)
                            / 100 * per_gg
              )
       )
 + round( ( sum(sum(gg_100) + sum(gg_per) )
          - ( sum( round( sum(gg_100) / P_gg_lavoro ) )
            + sum( round( sum(gg_per) / P_gg_lavoro ) )
            ) * P_gg_lavoro
          ) / P_gg_lavoro
        )
 / decode
