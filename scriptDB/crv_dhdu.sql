CREATE OR REPLACE VIEW DOTAZIONE_XORE_DIRITTO_UE ( UTENTE, 
...
CONTRATTISTI, SOVRANNUMERO,NUMERO ) AS select rido.UTENTE 
      ...
      ,round(decode( gp4_gest.get_ore_do(pegi.gestione) 
                    ,'U',sum( nvl( pegi.ore 
					             , gp4_cost.get_ore_lavoro( gp4_figi.GET_QUALIFICA(pegi.figura,pegi.dal),pegi.dal) 
                                 ) / gp4_cost.get_ore_lavoro_divisione( gp4_figi.GET_QUALIFICA(pegi.figura,pegi.dal),pegi.dal) 
                            ) 
					    ,count(ci) 
				   ),1 
			)			effettivi 
