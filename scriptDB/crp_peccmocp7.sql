CREATE OR REPLACE PACKAGE peccmocp7 IS
/******************************************************************************
 NOME:        PECCMOCP7
 DESCRIZIONE: Calcolo del Netto e determinazione delle Tariffe - Previsione
 ANNOTAZIONI: -
 REVISIONI:
 ...
 4    05/07/2004 MF     Calcolo Tariffa su voci di Movimenti Contabili con riferimento
                        ...
                        Introduzione richiami a Package PECCMOCP_TARIFFA.
******************************************************************************/
...
CREATE OR REPLACE PACKAGE BODY peccmocp7 IS
  ...
  BEGIN  --Emissione della voce di ARROTONDAMENTO ATTUALE
    ...
    where voec.automatismo in ( 'ARR_ATT'
      ...
      and (    voec.automatismo in ( 'ARR_ATT_S', 'ARR_ATT_SN' )
        or  /*In *PREVISIONE* e` sempre mese = 12
           ( P_mese != 12 or P_tipo != 'N') and
    */
...
/* fine modifica del 24/06/99 */
  END;
  peccmocp.VOCI_TOTALE  --Totalizzazione delle Voci di Arrotondamento
  ...
  <<tratta_voce>>
   ...
   BEGIN  --Trattamento voce a Tariffa
      ...
      IF P_tariffa != 0 THEN 
         ...
         END;
/* modifica del 16/03/2004 */
         D_tipo_riferimento := PECCMOCP_TARIFFA.TIPO_RIFERIMENTO
                                               ( D_rapporto
      ...
      -- AND nvl(P_specifica,' ') not in ('RGA13M','RGA13A') THEN -- /* modifica del 16/03/2004 */
         ...
         ELSE
            BEGIN  -- Rapporto Tariffa sul Periodo effettivo
               ...
               PECCMOCP_TARIFFA.RAPPORTO_PERIODO
                               ( P_ci
                               , P_fin_ela
                               , P_rif, P_tariffa, P_tar_eff
                               , P_gg_lavoro, P_base_ratei
