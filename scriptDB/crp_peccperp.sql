CREATE OR REPLACE PACKAGE PECCPERP IS
/******************************************************************************
 NOME:        Peccperp
 ...
 1    12/04/2007  CB    Aggiunto il controllo sul codice di competenza di RAIN sul C_SEL_RAGI
 1.1  14/11/2007  NN    Gestito il nuovo campo CARE.GG_DF, per il momento valorizzato con zero.
******************************************************************************/

revisione varchar2(30) := '1 del 14/11/2007';
...
END;
/
CREATE OR REPLACE PACKAGE BODY PECCPERP IS
...
BEGIN
 ...
 END;
 END IF;
 peccperp2.CARE_INSERT --Inserimento Periodi da Calcolare
         ( P_ci, P_dal, P_al
 ...
 peccperp3.PERE_INSERT --Inserimento Periodi da Retribuire
         ( P_ci, P_dal, P_al
         , P_cong_ind, P_d_cong, P_d_cong_al
 ...
 peccperp3.PERE_CHECK --Verifica giornate effettive
         ( P_ci, P_fin_ela, P_fin_per
         , P_trc, P_prn, P_pas, P_prs, P_stp, P_tim
         );
...
EXCEPTION
 ...
 RAISE FORM_TRIGGER_FAILURE;
END;
--Calcolo Periodi di Previsione
--
--Procedura di Calcolo Periodi di Previsione
--
PROCEDURE calcolo IS
          ...
          d_fin_per date; --Fine periodo di Calcolo *PREVISIONE*
                          --
CURSOR C_SEL_RAGI
     ( P_ci_start number
     ) IS
...
BEGIN
 ...
 --per iquali non si esegue il controllo di competenza
   d_cc:=null;
   END IF;
 END;
 ...
 RAISE TIMEOUT_ON_RESOURCE;
  ...
  CALCOLO_CI  --Calcolo periodi individuali
           ...
           , D_anno, D_mese, D_mensilita
           , D_ini_ela, D_fin_ela, D_ini_per, D_fin_per
 ...
 EXCEPTION
  WHEN FORM_TRIGGER_FAILURE THEN
   ...
   peccperp.errore := 'P05809';  --Errore in Elaborazione
 ...
 WHEN TIMEOUT_ON_RESOURCE THEN
  ...
  log_trace(8,D_prn,D_pas,D_prs,D_stp,0,D_tim_ci);
  IF peccperp.errore != 'P05809' THEN  --Errore in Elaborazione
	 peccperp.errore := 'P05808';  --Segnalazione in Elab.
  END IF;
  commit;
  CLOSE C_UPD_RAGI;
  END tratta_ci;
 ...
 IF peccperp.errore != 'P05809' AND --Errore in Elaborazione
    peccperp.errore != 'P05808' AND --Segnalazione in Elab.
    peccperp.errore != 'P05828' THEN  --Giornate da Verificare
    peccperp.errore := 'P05802';  --Elaborazione Completata
 END IF;
 commit;
  END;
 END ciclo_ci;
...
END CALCOLO;
  ...
  begin
   ...
   IF w_prenotazione != 0 THEN
      IF peccperp.errore = 'P05808' THEN
      ...
      ELSIF
         ...
         commit;
      ELSIF
         substr(peccperp.errore,6,1) = '9' THEN
         update a_prenotazioni
            set errore       = 'P05809'
              , prossimo_passo = 91
          where no_prenotazione = w_prenotazione
