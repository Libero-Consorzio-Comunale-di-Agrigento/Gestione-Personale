CREATE OR REPLACE PACKAGE peccinep2 IS
/******************************************************************************
 NOME:          PECCINEP2
 DESCRIZIONE:   
 ARGOMENTI:   
...
END;
/
CREATE OR REPLACE PACKAGE BODY peccinep2 IS
...
/* fine modifica del 26/06/95 */
 BEGIN  -- Aggiorna Data AL su Informazioni Base su Totale
  ...
  update informazioni_retributive_bp x
 set al =
...
/* fine modifica del 26/06/95 */
 ...
 ELSE
 --
 -- Un solo Codice Individuale
 --
/* modifica del 26/06/95 */
 BEGIN  -- Inserisce voci Totale per informazioni Base su Totale
  P_stp := 'INRE_BASE_TOT_INS.a';
 FOR CURS in
 (select distinct
 inre.ci
 , tovo.voce_acc
 ...
 * nvl(bavo.moltiplica,1)
 ...
 from informazioni_retributive_bp inre
  ...
  , qualifiche_giuridiche qugi
  where pegi.ci  = P_ci
  and least( nvl(inre.al,P_nullal)
 , nvl(pegi.al,P_nullal)
 , nvl(qugi.al,P_nullal)
 , nvl(vbvo.al,P_nullal)
 )  >= P_dal
