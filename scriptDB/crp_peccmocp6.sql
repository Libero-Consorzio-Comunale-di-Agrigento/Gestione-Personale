CREATE OR REPLACE PACKAGE peccmocp6 IS
/******************************************************************************
 NOME:        PECCMOCP6
 DESCRIZIONE: Determinazione Voci a Quantità e a Calcolo - Previsione
 ANNOTAZIONI: -
 REVISIONI:
 ...
 4.1  23/11/2004 NN     Non valorizzava la qta per 13A con riferimento a mesi
                        dell'anno ma precedenti al mese di elaborazione 
 ...
 6    28/06/2005 CB     Gestione codice_siope
******************************************************************************/

revisione varchar2(30) := '6 del 28/06/2005';
...
CREATE OR REPLACE PACKAGE BODY peccmocp6 IS
...
/* fine modifica 16/03/2004 */
     where tavo.voce = P_voce
      and P_rif between nvl(tavo.dal,to_date(2222222,'j'))
...
/* fine modifica del 26/10/2004 */ 
    ...
    and (   r.competenza  = lower(r.competenza)
     ...
     --
/* modifica del 23/11/2004 */
--    and r.periodo = P_fin_ela
  and r.periodo = (select max(r2.periodo)
                     from periodi_retributivi_bp r2
                    where r2.periodo >= to_date('01/01/'||to_char(P_d_cong,'yyyy'),'dd/mm/yyyy')
                      and r2.periodo <= P_fin_ela
                      and r2.ci       = P_ci
                      and r2.al       = r.al)
/* fine modifica del 23/11/2004 */
/* 
        between decode
        ...
        IF substr( P_estrazione,1,1) = 'C' AND
          ...
          VOCI_CALC_QTA  --Determinazione Quantita`
             ( P_ci, P_al
                  ...
                  , P_fin_ela, P_d_cong, P_estrazione
...
/* fine modifica 16/03/2004 */
        ...
        OR substr(P_estrazione,1,1) = 'Q'  AND
          ...
          BEGIN  --Determinazione del valore della Tariffa
             peccmocp7.VOCI_CALC_TAR  --Calcolo della Tariffa
               ( P_ci, P_al
...
BEGIN
  ...
  VOCI_CALC_VAL  --Determinazione Quantita` e Calcolo Tariffa
   ( P_ci, P_al
      , P_anno, P_mese, P_mensilita, P_fin_ela, P_d_cong, P_estrazione
      , D_numero_voci, P_base_ratei
      , P_trc, P_prn, P_pas, P_prs, P_stp, P_tim
   );
  peccmocp.log_trace(P_trc,P_prn,P_pas,P_prs,P_stp,SQL%ROWCOUNT,P_tim);
  IF D_numero_voci != 0 THEN
   peccmocp.VOCI_TOTALE  --Totalizzazione delle Voci a Calcolo
...
;
  ...
  VOCI_CALC_VAL  -- Calcolo Tariffe voci a QUANTITA' non valorizzate
    ( p_ci, p_al
        , p_anno, p_mese, p_mensilita , p_fin_ela, p_d_cong, 'Q'
        , D_numero_voci, P_base_ratei
        , P_trc, P_prn, P_pas, P_prs, P_stp, P_tim
    );
  peccmocp.log_trace(P_trc,P_prn,P_pas,P_prs,P_stp,SQL%ROWCOUNT,P_tim);
  IF D_numero_voci != 0 THEN
    peccmocp.VOCI_TOTALE  -- Totalizzazione delle Voci a Quantita`
