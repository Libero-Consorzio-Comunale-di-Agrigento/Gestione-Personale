CREATE OR REPLACE PACKAGE CURSORE_FISCALE IS
/******************************************************************************
 NOME:          CURSORE_FISCALE 
 DESCRIZIONE:   
 ARGOMENTI:
 RITORNA:
 ECCEZIONI:
 ...
 1    10/08/2003       
 2    01/07/2004 CB 
 3    10/06/2005 GM     Estrazione nuovi campi RIT_INPDAP, RIT_TFS, CONTR_TFR     
 3.1  10/10/2005 ML     Modificato cur_inpdap per errore in determinate versioni oracle (A12942)
 3.2  12/11/2006 MS     Modifica per Adeguamento CUD/2006 ( att. 13954 )
 3.3  25/01/2006 MS     Correzioni su segnalazioni varie ( att. 14619 )
 3.4  30/01/2006 MS     Correzioni da test ( att. 13956 )
 3.5  31/01/2006 MS     Correzioni da test ( att. 13956 )
 3.6  22/02/2006 MS     Mod. cassa previdenza ( Att. 15018 )
 3.7  27/02/2006 MS     Mod. Estrazione dati EMENS ( Att. 15079 )
 3.8  14/03/2006 MS     Mod. gestione Eliminativi EMENS ( Att. 15386 )
 3.9  03/04/2006 MS     Mod. estrazione dati ( att. 15600 )
 3.10 07/04/2006 MS     Mod. coll. agevolati ( att. 15658 )
 4.0  22/05/2006 MS     Revisione per 770/2006 ( att. 16250 )
 4.1  05/09/2006 MS     Modifiche cursore DMA ( att. 17255 )
 4.2  11/09/2006 MS     Modifica lettura posizione da vista ( att. 17539 )
 5.0  19/01/2007 MS     Modifica per Adeguamento CUD/2007 ( att. 19136 + 16555 )
 5.1  16/02/2007 MS     Gestione su CUD Rapporti Precedenti con Liq. ( 17900.1 )
 5.2  07/03/2007 MS     Modifica cursore DMA per rec. con importi a 0 ( 20042 )
 5.3  14/03/2007 AD     Gestione CUD da Portale
 5.4  04/09/2007 MS     Modifica cursore DMA per cassa pensione e previdenza ( 22626 )
******************************************************************************/
  cursor CUR_RPFA2 
  ( P_anno         number
  , P_filtro_1     varchar2
...
-- Dipendenti con previdenziale e fiscale 
  select  max(distinct rpfa.s1)                                          s1
        ...
        , max(distinct decode(RAIND.nome,'',1,0)||RAIND.cognome||'  '||RAIND.nome
                     ||decode(RAIND.ci,RAIN.ci,1,decode(RAIND.ci,RADI.ci,0,2))
        ...
        , max(distinct substr(to_char(anag.data_nas,'ddmmyyyy'),1,8))    data_nas
        , max(distinct anag.sesso)                                       sesso
        , max(distinct anag.comune_nas)                                  comune_nas
        , max(distinct anag.provincia_nas)                               provincia_nas
        , max(distinct rpad(nvl( P_cod_fis , ltrim(nvl( gest.codice_fiscale,gest.partita_iva))
                               ),16))                                    ente_cf
        , 'NO'                                                           flag_solo_liq
  	from gestioni            gest
       , anagrafici            anag
  ...
  union
-- Precedenti Rapporti con l'ente da trattare sezione liquidazione
  select  max(distinct rpfa.s1)                                          s1
        , max(distinct rpfa.s2)                                          s2
        , max(distinct rpfa.s3)                                          s3
        , max(distinct rpfa.s4)                                          s4
   ...
   where gest.codice (+)          = rpfa.c1
     ...
     and anag.al                 is null
     and radi.ci                  = rpfa.ci
     and raind.ci                 = rain.ci
     and rain.rapporto            = clra.codice
     and clra.cat_fiscale        in ('1','10','15','2','20','25')
     and radi.ci_erede            = rain.ci
     and radi.rilevanza           = 'R'
     and radi.anno                = P_anno
     and defs.ci                  = radi.ci_erede
     and defs.anno                = P_anno
     and defs.rilevanza           = 'T'     
     and nvl(defs.c69,0) + nvl(defs.c71,0) != 0
     and nvl(P_estrazione_i,'1')  = '1'
     AND EXISTS
         (SELECT 'x'
            FROM RAPPORTI_INDIVIDUALI rain
...
cursor CUR_DMA
  ...
  ( select dma.gest_cf            gest_cf
         ...
         , decode( sum(nvl( dma.ipn_tfs,0))+sum(nvl( dma.ipn_tfr,0))
                 , 0, null
      ...
      from ( select gest.codice_fiscale       gest_cf
          ...
          group by gest.codice_fiscale 
             ...
             having abs(decode(P_decimali
                          ,'X', round(sum(nvl(ddma.ipn_pens_periodo,0)),2)*100
                              , round(sum(nvl(ddma.ipn_pens_periodo,0)),0)
                          ))
                  + abs(decode(P_decimali
                          ,'X', round(sum(nvl(ddma.contr_pens_periodo,0) + nvl(contr_su_eccedenza,0)),2)*100
                              , round(sum(nvl(ddma.contr_pens_periodo,0) + nvl(contr_su_eccedenza,0)),0)
                          ))
                  + abs(decode(P_decimali
                  ...
                  + abs(decode( sum(nvl(ddma.contr_enpdedp,0))
                           ,0, 0
                             , decode(P_decimali
                                     ,'X', round(sum(nvl(ddma.ipn_cassa_credito,0)),2)*100
                                         , round(sum(nvl(ddma.ipn_cassa_credito,0)),0)
...
group by dma1.gest_cf
       ...
       , dma1.anno_rif
       , dma1.cassa_credito
order by dma1.gest_cf     asc
       , dma1.prog_inpdap asc
       , decode(dma1.anno_rif,P_anno,9999,dma1.anno_rif) desc
--       , decode(dma1.anno_rif,P_anno,1000,dma1.anno_rif) asc
       , nvl(dma1.cassa_pensione,0)   asc
       , nvl(dma1.cassa_previdenza,0) asc
       , nvl(dma1.cassa_credito,0)    asc
       , nvl(dma1.cassa_enpdedp,0)    asc
;
...
END; 
/
CREATE OR REPLACE PACKAGE BODY cursore_fiscale IS
 FUNCTION VERSIONE  RETURN VARCHAR2 IS
 BEGIN
    RETURN 'V5.4 del 04/09/2007';
 END VERSIONE;
