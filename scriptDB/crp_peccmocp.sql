CREATE OR REPLACE PACKAGE peccmocp IS
/******************************************************************************
 NOME:        PECCMOCP
 ...
 3    28/06/2005 CB     Gestione Codice SIOPE 
 3.1  12/04/2007 CB     Aggiunto il controllo sul codice di competenza di RAIN sul C_SEL_RAGI
 3.2  25/07/2007  AM     Adeguamento per gestione RADI
******************************************************************************/

revisione varchar2(30) := '3.2 del 25/07/2007';
...
END;
/
CREATE OR REPLACE PACKAGE BODY peccmocp IS
...
/* fine modifica del 28/04/2004 */
         , caco.risorsa_intervento, caco.capitolo, caco.articolo, caco.conto
         , caco.impegno, caco.anno_impegno, caco.sub_impegno, caco.anno_sub_impegno
         , caco.anno_del, caco.sede_del, caco.numero_del, caco.delibera ,caco.codice_siope
   ;
...
BEGIN
  ...
  COMMIT;
END;
-- Calcolo Elaborazione di Previsione
--
-- Procedura di Calcolo Movimenti Contabili di Previsione
--
PROCEDURE calcolo
IS
 ...
 BEGIN
  ...
  BEGIN  -- Ciclo su Individui
     ...
     BEGIN
        ...
        EXCEPTION
          WHEN NO_DATA_FOUND THEN
-- Posti vacanti per cui non si effettua il controllo sulla competenza
        ...
        BEGIN  -- Calcolo Individuo
          Peccmocp1.CALCOLO_CI  -- Calcolo voci retributive individuali
             ( ragi.ci, D_ni
             , LEAST( NVL(ragi.al,TO_DATE('3333333','j'))
                   , to_date('3112'||to_char(D_anno),'ddmmyyyy')
             ...
             , D_tipo, D_mens_codice, D_periodo
             , D_ass_fam
